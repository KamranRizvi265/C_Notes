...
Node * delete_last(Node *head)
{
    ...
    while(q->next != NULL)  // Traverse the list until the last node
    {
        p = p->next;
        q = q->next;
    }

    p->next = NULL;  // Set the next of the second last node to NULL
...
Node * delete_value(Node *head, int value)
{
    ...
    Node *p = head;
    Node *q = head->next;
    while(q->data != value && q->next != NULL)  // Traverse the list until the value is found or the end is reached
    {
        p = p->next;
        q= q->next;
    }
